...
console.log(listing1.currentOwner);
...
function realtorFees(listing) {
  let priceNumber = parseFloat(
    listing.price.replaceAll("$", "").replaceAll(",", "")
  );
  let percentage = priceNumber <= 450000 ? 0.025 : 0.02;
  return Math.round(priceNumber * percentage);
}
...
listingAscendingly.sort((a, b) => {
  let price0 = parseFloat(a.price.replaceAll("$", "").replaceAll(",", ""));
  let price1 = parseFloat(b.price.replaceAll("$", "").replaceAll(",", ""));
...
for (let i = 0; i < listings.length; i++) {
  ...
  let builtInText = listing.propertySummary.builtIn;
  if (builtInText.length === 4 && builtInText >= "1990") {
